...
import { getPurchaseOrdersExportRows } from "@/lib/data/purchaseOrders";
import { generateQuickExport, summarizeFilters, type QuickExportFormat } from "@/lib/quickExport";

export async function GET(req: Request) {
  const auth = await requireModuleAccess("purchaseOrders", "canExport");
  ...
  try {
    ...
    return new NextResponse(Buffer.from(bytes), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="purchase-orders-${Date.now()}.${extension}"`,

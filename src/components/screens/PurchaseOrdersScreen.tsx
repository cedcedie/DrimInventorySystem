...
function PurchaseOrdersScreenInner({ initialData }: { initialData?: PurchaseOrdersData }) {
  ...
  const canExport = useCan("purchaseOrders", "canExport");
  ...
  const { data, dataUpdatedAt, page, setPage } = usePaginatedQuery<PurchaseOrdersData>({
    queryKey: (p) =>
      queryKeys.purchaseOrders({ page: p, refNo: debouncedRefNo, supplier: debouncedSupplier, item: debouncedItem, filedBy: debouncedFiledBy }),
    url: (p) =>
      `/api/purchase-orders?page=${p}&refNo=${encodeURIComponent(debouncedRefNo)}&supplier=${encodeURIComponent(debouncedSupplier)}&item=${encodeURIComponent(debouncedItem)}&filedBy=${encodeURIComponent(debouncedFiledBy)}`,
    initialData,
    live: liveWarm,
  });
    ...
    <Box>
      ...
      {!data ? (
        ...
        <TableShell minWidth={780}>
          ...
          {data.rows.length === 0 && (
            <EmptyState
              message={hasFilter ? "No purchase orders match your search." : "No purchase orders yet — create one to track what's on order."}
              actionLabel={canCreate && !hasFilter ? "New Purchase Order" : undefined}
